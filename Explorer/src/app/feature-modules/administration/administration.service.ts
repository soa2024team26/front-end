...
export class AdministrationService {
  ...
  getById(message: Message): Observable<Profile> {
    return this.http.get<Profile>('http://localhost:8086/api/administration/profile/by-id/' + message.senderId);
  }

  getById2(message: Message): Observable<Profile> {
    return this.http.get<Profile>('http://localhost:8086/api/administration/profile2/by-id/' + message.senderId);
  }
  ...
  getProfileByUserId(id: number): Observable<Profile> {
    return this.http.get<Profile>('http://localhost:8086/api/administration/profile/get-profile-by-user'+ id);
  }

  getByUserId2(): Observable<Profile> {
    return this.http.get<Profile>('http://localhost:8086/api/administration/profile2/by-user');
  }
  
  updateProfile(profile: Profile): Observable<Profile> {
    return this.http.patch<Profile>('http://localhost:8086/api/administration/profile/' + profile.id + '/' + profile.userId, profile);
  }

  updateProfile2(profile: Profile): Observable<Profile> {
    return this.http.patch<Profile>('http://localhost:8086/api/administration/profile2/' + profile.id + '/' + profile.userId, profile)
      .pipe(
  ...
  upload(file: File): Observable<HttpEvent<any>> {
    ...
    const req = new HttpRequest('POST', `http://localhost:8086/api/administration/profile/UploadFile`, formData, {
      reportProgress: true,
      responseType: 'json'
    });
  ...
  upload2(file: File): Observable<HttpEvent<any>> {
    ...
    const req = new HttpRequest('POST', `http://localhost:8086/api/administration/profile2/UploadFile`, formData, {
      reportProgress: true,
      responseType: 'json'
    });
  ...
  updatePublicRequest(pr: PublicRequest): Observable<PublicRequest> {
    return this.http.put<PublicRequest>('http://localhost:8086/api/administrator/publicRequest/update/' + pr.id, pr);
  }
  ...
  updateMessage(message: Message): Observable<Message> {
    return this.http.put<Message>('http://localhost:8086/api/administration/message/' + message.id + '/' + message.senderId + '/' + message.receiverId, message);
  }

  updateMessage2(message: Message): Observable<Message> {
    return this.http.put<Message>('http://localhost:8086/api/administration/message2/' + message.id + '/' + message.senderId + '/' + message.receiverId, message);
  }

  //WALLET
  getWalletByUserId(): Observable<Wallet> {
    return this.http.get<Wallet>('http://localhost:8086/api/administrator/wallet/byUser');
  }

  getAllWallets(): Observable<PagedResults<Wallet>> {
    return this.http.get<PagedResults<Wallet>>('http://localhost:8086/api/administrator/wallet');
  }

  addAC(wallet: Wallet): Observable<Wallet> {
    return this.http.put<Wallet>('http://localhost:8086/api/administrator/wallet/' + wallet.id , wallet);
  }
  ...
  getQuestions(): Observable<Question[]> {
    return this.http.get<Question[]>(`http://localhost:8086/api/question/unanswered`);
  } 
  
  createAnswer(answer: Answer): Observable<Answer> {
    return this.http.post<Answer>('http://localhost:8086/api/answer/createAnswer', answer);
